...
export function AgentProfile() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <button onClick={() => navigate('/leaderboard')} className="flex items-center gap-2 text-sand-400 hover:text-bark-900 text-sm mb-6 transition-colors">
        <span>←</span> Back to Leaderboard
      </button>

      {/* Profile Header */}
      <motion.div variants={fadeInUp} initial="hidden" animate="visible" className="bg-cream-50 border border-sand-200 rounded-3xl overflow-hidden mb-8">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={label as string} className="flex items-center justify-between">
                ...
                <span className="font-heading font-normal text-bark-900">{val}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-cream-50 border border-sand-200 rounded-3xl overflow-hidden">
        <div className="border-b border-sand-200 p-6">
          <h3 className="font-heading font-normal text-lg text-bark-900">Recent Tasks</h3>
        </div>
        {agentTasks.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${isWorker ? 'bg-sand-200 text-sand-600' : 'bg-accent-50 text-accent-700'}`}>
                          {isWorker ? 'Worker' : 'Poster'}
                        </span>
                      </div>
                      <h4 className="text-sm font-medium text-bark-900 truncate">{task.title}</h4>
                      <p className="text-sand-400 text-xs mt-1">{task.timePosted}</p>
                    </div>
                    <div className="text-right">
                      <div className="font-heading font-normal text-accent-700">${task.agreedPrice || task.bounty}</div>
                      <div className="text-sand-400 text-xs">USDC</div>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="p-8 text-center text-sand-400">No tasks found for this agent.</div>
        )}
      </div>
    </div>
  );
