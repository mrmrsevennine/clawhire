...
export function Header({ onPostClick }: Props) {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-500 ${
      scrolled ? 'bg-cream-50/90 backdrop-blur-xl shadow-sm border-b border-sand-200' : 'bg-bark-950/80 backdrop-blur-sm'
    }`}>
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="flex items-center gap-2.5 group">
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none" className={`transition-colors ${scrolled ? 'text-bark-900' : 'text-cream-100'}`}>
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <button
            onClick={onPostClick}
            className="px-5 py-2.5 text-sm font-semibold rounded-full bg-accent-500 text-bark-950 hover:bg-accent-400 hover:shadow-lg hover:shadow-accent-500/20 transition-all duration-300 hover:-translate-y-0.5"
          >
            Post Task
          </button>
          <WalletButton />
        ...
        {/* Mobile */}
        <button onClick={() => setMobileOpen(!mobileOpen)} className={`md:hidden p-2 rounded-lg ${scrolled ? 'hover:bg-sand-100' : 'hover:bg-cream-100/10'}`}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={scrolled ? '#2A231D' : '#FFF9EE'} strokeWidth="2" strokeLinecap="round">
            {mobileOpen ? (
...
function NavLink({to,active,children,scrolled}:{to:string;active:boolean;children:React.ReactNode;scrolled?:boolean}) {
  const base = scrolled
    ? (active ? 'text-bark-900 bg-sand-100' : 'text-sand-500 hover:text-bark-900 hover:bg-sand-100')
    : (active ? 'text-cream-100 bg-cream-100/10' : 'text-cream-100/50 hover:text-cream-100 hover:bg-cream-100/5');
  return <Link to={to} className={`px-4 py-2 text-sm font-medium rounded-full transition-colors ${base}`}>{children}</Link>;
}

function MobileLink({to,active,onClick,children}:{to:string;active:boolean;onClick:()=>void;children:React.ReactNode}) {
  return <Link to={to} onClick={onClick} className={`px-4 py-3 text-sm font-medium rounded-2xl ${active?'text-bark-900 bg-sand-100':'text-sand-600 hover:bg-sand-100'}`}>{children}</Link>;
}
